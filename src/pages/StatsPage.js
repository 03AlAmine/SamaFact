...
// ── Lazy imports — au niveau du module, jamais dans le composant ─────────────
const LazyChart = React.lazy(() => import("react-apexcharts"));

const lazyNamed = (exportName) =>
  lazy(() =>
    import("../components/reports/Charts").then((m) => ({ default: m[exportName] }))
  );

const ClientChart            = lazyNamed("ClientChart");
const StatusChart            = lazyNamed("StatusChart");
const MonthlyComparisonChart = lazyNamed("MonthlyComparisonChart");
const PayrollChart           = lazyNamed("PayrollChart");
const EmployeChart           = lazyNamed("EmployeChart");
const RevenueChart           = lazyNamed("RevenueChart");
const ExpenseChart           = lazyNamed("ExpenseChart");
...
const StatsPage = ({ stats, allFactures, allAvoirs, allDevis, clients, payrolls, employees }) => {
  ...
  const advancedStats = useMemo(() => {
    ...
    const monthlyInvoices = Array(12).fill(0).map(() => ({
      count: 0, amount: 0, paid: 0, pending: 0, deposit: 0
    }));
    ...
    allFactures?.forEach(f => {
      ...
      if (date.getFullYear() === currentYear) {
        ...
        if (f.statut?.toLowerCase() === 'payé')        monthlyInvoices[month].paid    += amount;
        else if (f.statut?.toLowerCase() === 'en attente') monthlyInvoices[month].pending += amount;
        else if (f.statut?.toLowerCase() === 'acompte')    monthlyInvoices[month].deposit += amount;
      }
    });
    ...
    employees?.forEach(emp => {
      ...
      if (!departmentStats[dept]) {
        departmentStats[dept] = { count: 0, totalSalary: 0, averageSalary: 0 };
      }
    ...
    const topClients = [];
    const clientStats = {};
    const clientTypeStats = { client: 0, prospect: 0, fournisseur: 0 };
    ...
    Object.entries(clientStats)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .forEach(([name, amount]) => topClients.push({ name, amount }));

    clients?.forEach(c => {
      const type = c.type || 'client';
      if (clientTypeStats[type] !== undefined) clientTypeStats[type]++;
    });
    ...
    return {
      ...
      totalPaid, totalPending, totalDeposit, totalRevenue,
      monthlyInvoices, contractTypes, totalPayroll, averageSalary,
      topClients,
      collectionRate: totalPaid + totalDeposit > 0
        ? ((totalPaid + totalDeposit) / (totalPaid + totalDeposit + totalPending) * 100).toFixed(1)
        : 0,
      departmentStats, clientTypeStats, growthRate,
  ...
  const revenueChartOptions = useMemo(() => ({
    series: [
      { name: "Chiffre d'affaires", data: advancedStats.monthlyInvoices.map(m => m.amount) },
      { name: "Objectif",           data: advancedStats.monthlyInvoices.map(m => m.amount * 1.2) }
    ],
    chart: {
    ...
    yaxis: {
      labels: { formatter: val => val.toLocaleString() + " FCFA", style: { fontSize: "12px" } },
  ...
  const collectionChartOptions = useMemo(() => ({
    series: [
      { name: "Payé",       data: advancedStats.monthlyInvoices.map(m => m.paid) },
      { name: "En attente", data: advancedStats.monthlyInvoices.map(m => m.pending) },
      { name: "Acompte",    data: advancedStats.monthlyInvoices.map(m => m.deposit) }
    ],
    chart: { height: 350, type: "bar", stacked: true, toolbar: { show: true }, foreColor: "#9aa0ac" },
  ...
  const contractChartOptions = useMemo(() => ({
    ...
    chart: { height: 300, type: "donut", foreColor: "#9aa0ac", toolbar: { show: true } },
    ...
    plotOptions: {
      pie: {
        donut: {
          ...
          labels: {
            show: true,
            total: { show: true, label: "Total", formatter: () => employees?.length || 0 }
          }
        }
      }
    },
    ...
    tooltip: { y: { formatter: val => `${val} employés` } }
  }), [advancedStats.contractTypes, employees?.length]);

  const topClientsChartOptions = useMemo(() => ({
    series: [{ data: advancedStats.topClients.map(c => c.amount) }],
    chart: { height: 300, type: "bar", toolbar: { show: true }, foreColor: "#9aa0ac" },
    ...
    dataLabels: {
      ...
      offsetX: 20,
      style: { colors: ["#fff"], fontSize: "11px", fontWeight: "bold", textShadow: "0 1px 2px rgba(0,0,0,0.3)" }
    },
    xaxis: {
  ...
  // ── moduleConfig mémoïsé — évite de recréer tous les JSX à chaque render ──
      ...
      charts: [
        {
          ...
          icon: <FaChartLine />,
          comp: <Suspense fallback={<ChartSkeleton />}><LazyChart options={revenueChartOptions} series={revenueChartOptions.series} type="area" height={350} /></Suspense>,
          type: "revenue"
        },
        {
          title: "Recouvrement mensuel",
          icon: <FaChartBar />,
          comp: <Suspense fallback={<ChartSkeleton />}><LazyChart options={collectionChartOptions} series={collectionChartOptions.series} type="bar" height={350} /></Suspense>,
          type: "collection"
        },
        {
          title: "Top 5 Clients",
          icon: <FaChartPie />,
          comp: <Suspense fallback={<ChartSkeleton />}><LazyChart options={topClientsChartOptions} series={topClientsChartOptions.series} type="bar" height={300} /></Suspense>,
          type: "clients"
        },
        {
          title: "Statut des factures",
          icon: <FaChartPie />,
          comp: <Suspense fallback={<ChartSkeleton />}><StatusChart invoices={allFactures} showLegend={true} /></Suspense>,
          type: "status"
        },
        {
          title: "Comparaison annuelle",
          icon: <FaChartLine />,
          comp: <Suspense fallback={<ChartSkeleton />}><MonthlyComparisonChart invoices={allFactures} /></Suspense>,
          type: "comparison"
        },
        {
          title: "Répartition des clients",
          icon: <FaChartPie />,
          comp: <Suspense fallback={<ChartSkeleton />}><ClientChart clients={clients} showLegend={true} /></Suspense>,
      ...
      stats: [
        ...
        },
        {
          ...
          subStats: Object.entries(advancedStats.departmentStats)
            .slice(0, 2)
            .map(([dept, data]) => ({ label: dept, value: `${data.count} pers.` }))
        }
      ],
      charts: [
        {
          ...
          icon: <FaChartPie />,
          comp: <Suspense fallback={<ChartSkeleton />}><PayrollChart payrolls={payrolls} showLegend={true} /></Suspense>,
          type: "payroll"
        },
        {
          title: "Types de contrat",
          icon: <FaChartPie />,
          comp: <Suspense fallback={<ChartSkeleton />}><LazyChart options={contractChartOptions} series={contractChartOptions.series} type="donut" height={300} /></Suspense>,
          type: "contracts"
        },
        {
          title: "Répartition par département",
          icon: <FaUsers />,
          comp: <Suspense fallback={<ChartSkeleton />}><EmployeChart employees={employees} showLegend={true} groupBy="departement" /></Suspense>,
          type: "department"
        },
        {
          title: "Évolution masse salariale",
          icon: <FaChartLine />,
          comp: <Suspense fallback={<ChartSkeleton />}><RevenueChart data={payrolls} showTrend={true} /></Suspense>,
          type: "evolution"
        },
        {
          title: "Charges sociales",
          icon: <FaMoneyBillWave />,
          comp: <Suspense fallback={<ChartSkeleton />}><ExpenseChart data={payrolls} /></Suspense>,
          type: "charges"
        },
        {
          ...
          comp: (
            <Suspense fallback={<ChartSkeleton />}>
              <LazyChart
                options={{
                  series: [{ data: Object.values(advancedStats.departmentStats).map(d => Math.round(d.averageSalary)) }],
                  chart: { type: "bar", height: 300, toolbar: { show: false }, foreColor: "#9aa0ac" },
                  plotOptions: { bar: { horizontal: true, borderRadius: 6, distributed: true } },
                  xaxis: { categories: Object.keys(advancedStats.departmentStats) },
                  yaxis: { labels: { formatter: val => formatNumber(val) + " FCFA" } },
                  colors: ["#4f46e5", "#10b981", "#f59e0b", "#ef4444", "#8b5cf6"],
                  dataLabels: { enabled: false },
                  tooltip: { y: { formatter: val => formatNumber(val) + " FCFA" } }
                }}
                series={[{ data: Object.values(advancedStats.departmentStats).map(d => Math.round(d.averageSalary)) }]}
                type="bar"
                height={300}
    ...
    >
      ...
      <motion.div className="mf-stats-header" variants={itemVariants}>
        ...
        <div className="mf-stats-toolbar">
          ...
          <div className="mf-export-dropdown">
            <button className="mf-export-btn" onClick={() => setShowExportMenu(!showExportMenu)}>
              <FaDownload /> Exporter
            </button>
            <AnimatePresence>
              {showExportMenu && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                >
                  <button onClick={exportToExcel}><FaFileExport /> Excel (.xlsx)</button>
                  <button onClick={exportToPDF}><FaFileExport /> PDF</button>
                  <button onClick={printReport}><FaPrint /> Imprimer</button>
                  <button onClick={shareReport}><FaShare /> Partager</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <motion.div className="mf-stats-filters" variants={itemVariants}>
        {["month", "quarter", "year"].map(period => (
          <button
            key={period}
            className={`mf-filter-btn ${selectedPeriod === period ? "mf-active" : ""}`}
      ...
      <motion.div className="mf-kpi-summary" variants={itemVariants}>
        <div className="mf-summary-card">
          <div className="mf-summary-icon mf-summary-total"><FaMoneyBillWave /></div>
          <div className="mf-summary-content">
            <span className="mf-summary-label">
              Total {activeModule === "mentafact" ? "CA" : "Masse salariale"}
            </span>
            <span className="mf-summary-value">
              {formatNumber(advancedStats.totalRevenue || advancedStats.totalPayroll)} FCFA
            </span>
        ...
        <div className="mf-summary-card">
          <div className="mf-summary-icon mf-summary-count"><FaUsers /></div>
          <div className="mf-summary-content">
            <span className="mf-summary-label">
              {activeModule === "mentafact" ? "Clients" : "Employés"}
            </span>
            <span className="mf-summary-value">
              {activeModule === "mentafact" ? stats.totalClients : stats.totalEmployees}
            </span>
            <span className="mf-summary-trend mf-trend-up"><FaArrowUp /> +12%</span>
          </div>
        </div>
        <div className="mf-summary-card">
          <div className="mf-summary-icon mf-summary-rate"><FaPercent /></div>
          <div className="mf-summary-content">
            ...
            <span className="mf-summary-trend mf-trend-up"><FaArrowUp /> +3%</span>
          </div>
        </div>
        <div className="mf-summary-card">
          <div className="mf-summary-icon mf-summary-docs"><FaFileInvoiceDollar /></div>
          <div className="mf-summary-content">
            <span className="mf-summary-label">Documents</span>
            <span className="mf-summary-value">
              {stats.totalFactures + stats.totalDevis + stats.totalAvoirs}
            </span>
            <span className="mf-summary-trend mf-trend-neutral">Stable</span>
          </div>
        </div>
      </motion.div>
          ...
          React.isValidElement(stat) ? (
            ...
            >
              ...
              <div className="mf-stat-info">
                ...
                {stat.trend && (
                  <div className={`mf-stat-trend ${stat.trendClass}`}>{stat.trend}</div>
                )}
              </div>
            </motion.div>
          )
            ...
            >
              <div className="mf-chart-header">
                ...
                <div className="mf-chart-actions">
                  <button className="mf-chart-action" title="Exporter" onClick={() => handleChartExport(chart.type)}>
                    <FaDownload />
                  </button>
                  <button className="mf-chart-action" title="Agrandir" onClick={() => toggleFullscreen(chart)}>
                    <FaEye />
                  </button>
                </div>
              </div>
      ...
      <motion.div className="mf-performance-indicators" variants={itemVariants}>
        <div className="mf-performance-header">
          ...
          <span className="mf-performance-update">
            <FaRegCalendarAlt /> Mise à jour en temps réel
          </span>
        </div>
        <div className="mf-indicators-grid">
          <div className="mf-indicator-card">
            <div className="mf-indicator-icon mf-indicator-success"><FaChartLine /></div>
          ...
          <div className="mf-indicator-card">
            <div className="mf-indicator-icon mf-indicator-warning"><FaMoneyBillWave /></div>
            <div className="mf-indicator-content">
          ...
          <div className="mf-indicator-card">
            <div className="mf-indicator-icon mf-indicator-info"><FaUsers /></div>
            <div className="mf-indicator-content">
          ...
          <div className="mf-indicator-card">
            <div className="mf-indicator-icon mf-indicator-danger"><FaFileSignature /></div>
            <div className="mf-indicator-content">
          ...
          >
            <div className="mf-modal-content">
              <div className="mf-modal-header">
                ...
                <button onClick={() => setIsFullscreen(false)}><FaTimes /></button>
              </div>
