...
import { useAppContext } from "../contexts/AppContext"; // Importez le contexte
import { FaUsers, FaFileInvoiceDollar, FaPlus, FaBolt, FaUserPlus, FaFileImport, FaMoneyBillWave, FaFileSignature } from "react-icons/fa";
import { InvoiceChart, EmployeChart, PayrollChart, ContractTypeChart, ClientChart } from "../components/Charts"; // Ajoutez PayrollChart
import { DocumentSliderCard, MonthlyAmountSliderCard, PaymentStatusSliderCard } from '../components/DocumentSliderCard';
...
const DashboardPage = ({ stats, allFactures, allDevis, allAvoirs, navigate, employees, payrolls, clients }) => {
    ...
    const { activeModule } = useAppContext(); // Récupérez le module actif
    ...
    // Fonction pour déterminer si un élément doit être affiché
    const shouldShow = (elementType) => {
        const visibilityRules = {
            // Éléments communs aux deux modules
            employees: true,
            payroll: activeModule === 'payroll',
            factures: activeModule === 'mentafact',
            devis: activeModule === 'mentafact',
            avoirs: activeModule === 'mentafact',
            quickActions: true,
            charts: true
        };
        return visibilityRules[elementType];
    };
        ...
        <>
            <div className="stats-grid">
                ...
                {activeModule === 'mentafact' ? (
                    ...
                    <div className="stat-card">
                        <div className="stat-icon masse_salariale">
                            <FaMoneyBillWave />
                        </div>
                        <div className="stat-info">
                            <h3>
                                {(payrolls || [])
                                    .reduce((sum, p) => sum + (parseFloat(p?.netAPayer) || 0), 0)
            ...
            <div className="quick-actions-grid">
                ...
                {activeModule === 'payroll' && (
                    ...
                    <button onClick={() => navigate("/bill")} className="quick-action">
                        <FaBolt className="action-icon" />
                        <span>Bulletin Express</span>
                    </button>
                )}
                {activeModule === 'mentafact' && (
                    <button onClick={() => navigate("/employee/new")} className="quick-action">
                        <FaUserPlus className="action-icon" />
                        <span>Nouvel Client</span>
                    </button>
                )}
                {activeModule === 'payroll' && (
                    <button onClick={() => navigate("/employee/new")} className="quick-action">
            ...
            <div className="charts-row">
                {activeModule === 'mentafact' ? (
                    ...
                    <>
                        <div className="chart-card">
                            <h3>Répartition des paies</h3>
                            <PayrollChart payrolls={payrolls} />
                        </div>
                        <div className="chart-card">
                            <h3>Types de contrat</h3>
                            <ContractTypeChart employees={employees} />
                        </div>
                    </>
                )}
